...
/// returned by `read_cursor_ide_access_token`; the picker uses metadata keys.
...
fn cursor_ide_db_path() -> Result<PathBuf, String> {
    let home = dirs_home().ok_or_else(|| "Could not resolve the home directory.".to_string())?;
...
#[tauri::command]
fn cursor_ide_login_meta() -> Result<Option<CursorIdeLogin>, String> {
    let path = cursor_ide_db_path()?;
...
#[tauri::command]
fn read_cursor_ide_access_token() -> Result<String, String> {
    let path = cursor_ide_db_path()?;
    query_item_table(&path, "cursorAuth/accessToken")?.filter(|s| !s.is_empty()).ok_or_else(|| {
...
#[cfg(test)]
mod cursor_ide_tests {
    use super::{query_item_table, sqlite3_bin};
    use std::process::Command;

    #[test]
    fn reads_item_table_value() {
        let dir = std::env::temp_dir().join(format!("usage-monitor-cursor-ide-{}", std::process::id()));
        ...
        assert_eq!(
            query_item_table(&db, "cursorAuth/cachedEmail").unwrap().as_deref(),
            Some("ada@example.com")
        );
        assert_eq!(
            query_item_table(&db, "cursorAuth/accessToken").unwrap().as_deref(),
            Some("header.payload.sig")
        );
        assert_eq!(query_item_table(&db, "cursorAuth/stripeMembershipType").unwrap(), None);
